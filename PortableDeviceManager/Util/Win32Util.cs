...
namespace PortableDeviceManager.Util
{
    internal static class Win32Util
    {
        ...
        public static void check_for_dialogs_thread() {
            EnumProcessWindows find_windows = new EnumProcessWindows();
            HashSet<IntPtr> processed_windows_ = new HashSet<IntPtr>();

            while (true) {
                var check_sleep_ms = PDManager.Instance.AutoCloseWinDialogs ? 50 : 500;
                Thread.Sleep(check_sleep_ms);
                if (PDManager.Instance.AutoCloseWinDialogs) {
